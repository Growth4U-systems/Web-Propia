...
export default function LinkedInBotPage() {
  ...
  async function approveCandidate(id: string) {
    ...
    const enrichPromise = fetch(`${API_BASE}/.netlify/functions/li-scrape?action=enrich-prospect`, {
      method: 'POST',
...
function OverviewTab({
  ...
  async function handleScrape() {
    ...
    try {
      ...
      const startRes = await fetch(`${API_BASE}/.netlify/functions/li-scrape?action=start`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      for (let i = 0; i < 60; i++) {
        ...
        const statusRes = await fetch(`${API_BASE}/.netlify/functions/li-scrape?action=status&runId=${runId}`);
      ...
      let hasMore = true;
      while (hasMore) {
        const processRes = await fetch(`${API_BASE}/.netlify/functions/li-scrape?action=process`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  async function handleProspectScan() {
    ...
    try {
      ...
      const posts: { postUrl: string; creatorName: string; commentDraft?: string; postSnippet?: string }[] = [];
      ...
      const startRes = await fetch(`${API_BASE}/.netlify/functions/li-prospect?action=start`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      for (let i = 0; i < 60; i++) {
        ...
        const statusRes = await fetch(
          `${API_BASE}/.netlify/functions/li-prospect?action=status&reactionsRunId=${reactionsRunId}&commentsRunId=${commentsRunId}`
        );
        const statusData = await statusRes.json();
        if (statusData.finished) {
          finished = true;
      ...
      let hasMore = true;
      while (hasMore) {
        const processRes = await fetch(`${API_BASE}/.netlify/functions/li-prospect?action=process`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
                  ...
                  <div className="border-t border-slate-100 bg-slate-50/50 p-5 space-y-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <button
                          onClick={async () => {
                            const res = await fetch(`${API_BASE}/.netlify/functions/li-scrape?action=connection-msg`, {
                              method: 'POST',
                              headers: { 'Content-Type': 'application/json' },
