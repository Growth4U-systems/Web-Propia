 ...
 * blog). Cada página lo personaliza con un bloque de config, NUNCA copiando el JS.
 * Sin bloque de config, se comporta exactamente como el diagnóstico de siempre.
 *
 *   <script type="application/json" id="g4uq-config">
 *   {
 *     "source":  "LM-sistema-de-atribucion",  // <- lo que llega a GHL. UNO por pieza.
 *     "mode":    "gate",                      // "full" = 7 preguntas · "gate" = solo captura
 *     "gauge":   false,                       // el medidor solo aplica al diagnóstico
 *     "intro":   { "eyebrow": "…", "title": "…", "sub": "…", "cta": "…", "hint": "…" },
 *     "capture": { "title": "…", "sub": "…", "recibir": ["…","…","…"], "cta": "…" },
 *     "done":    { "title": "…", "sub": "…" },
 *     "waMsg":   "Quiero el sistema de atribución."
 *   }
 *   </script>
 *
...
(function () {
  ...
  // Config de la página. Sin <script id="g4uq-config">, todo cae a estos defaults.
  var CFG = (function () {
    var d = {
      // Aterriza en el campo `source` del contacto de GHL. Una pieza = un source.
      // Prefijo + slug normalizado (minúsculas, sin acentos, kebab-case):
      //   LM-sistema-de-atribucion · ART-unit-economics · BLOG-geo-fintechs · DIAG-trust-score
  ...
  // Lead magnet: sin las 7 preguntas ni web. Un muro de 7 preguntas para descargar un PDF
  // hunde la conversión. En gate NO se llama al Trust Core (no hay web que analizar).
  var GATE = CFG.mode === "gate";
  var MAX_COMP = 5; // competidores que puede añadir el usuario a mano
  ...
  /* CSS aditivo del hero, el gauge y los pasos anchos. El CSS base del quiz vive en el
   * rawHtml de Alarife y se deja intacto; esto viaja con su markup en el mismo archivo. */
  function injectCss() {
    ...
    st.textContent =
      ...
      "#g4uq .g4uq-hero.g4uq-hero-solo{grid-template-columns:1fr}" +
      ...
      /* Paso de la web: más compacto, que con 5 competidores se hacía largo. */
      ...
      /* Último paso: 2 columnas (contexto a un lado, formulario al otro) y compacto. */
      ...
      "#g4uq .g4uq-recibir{background:rgba(36,28,22,.03);border:1.5px solid rgba(36,28,22,.18);border-radius:9px;padding:11px 14px;margin-bottom:0}" +
  ...
  /* Gauge ilustrativo de la intro. El 74 es un EJEMPLO: en la intro el usuario aún no
   * tiene score, y el caption lo dice.
   *
   ...
   * stop-color). Con el atributo `d` revertido el path se queda sin geometría
   * (getTotalLength() = 0) y no se pinta nada. Solo el style inline sobrevive.
  ...
  // El "quiero X" concreto lo pone cada página vía CFG.waMsg.
  function waText() {
  ...
  function setProg() {
    ...
    // En gate el recorrido es intro -> captura -> listo, así que no valen los tramos del quiz.
    var pmap = GATE
  ...
  function render() {
    ...
    // al lado). Las 7 preguntas se quedan estrechas a propósito: una decisión a la vez.
    // Sin gauge (lead magnet) la intro no tiene nada que enseñar, así que se queda estrecha.
    ...
    if (s.type === "webinput") {
      // Se mantienen los dos modos de main ("Los añado yo" / "Elegidlos por mí"), que son
      // mucho más claros que el checkbox diminuto de antes. Lo que cambia: los competidores
      // ahora se pueden añadir y quitar hasta MAX_COMP, y solo se pide el NOMBRE (el
      // servidor ya resuelve nombre -> dominio).
      ...
      stage.innerHTML = '<div class="g4uq-webstep">' +
        ...
        '<div class="g4uq-field" style="margin-bottom:7px"><label>¿Y tus competidores?</label></div>' +
        '<div class="g4uq-opts" style="margin-bottom:13px">' +
        ...
        '<div id="comp-wrap"' + (mode === "mios" ? '' : ' style="display:none"') + '>' +
          '<div class="g4uq-field" style="margin-bottom:7px"><span class="g4uq-fieldhint">Solo el nombre. Puedes añadir hasta ' + MAX_COMP + '.</span></div>' +
      ...
      var w = document.getElementById("f-web"), b = document.getElementById("g4uq-next"),
          wrap = document.getElementById("comp-wrap"),
      ...
      function renderComps() {
        ...
        // Solo se re-renderiza al añadir/quitar. En oninput únicamente se actualiza el
        // array, para que el input no pierda el foco mientras se escribe.
      ...
      b.onclick = function () {
        ...
        S.competidores = S.compmode === "mios"
          ? comps.map(function (x) { return String(x || '').trim(); }).filter(Boolean)
          : [];
        launchTrustCore();
        analyzing(S.web);
        go(idOf("segmento"));
    ...
    if (s.type === "capture") {
      ...
      // La copy la manda la página: en un lead magnet tiene que hablar del contenido que se
      // descarga, no del Trust Score. Y solo se pide el NOMBRE: el apellido no se usaba para
      // nada salvo saludar, y alargaba el formulario.
      var preview = noweb
      ...
      function sync() {
        ...
        if (ok) {
          if (noweb) { go1.href = C.CAL; go1.target = "_blank"; go1.rel = "noopener"; }
            ...
            go1.href = "https://wa.me/" + C.WA + "?text=" + encodeURIComponent(waText());
      ...
      // Guardamos el canal elegido para no re-ofrecer en el frame final el que descartaron.
    ...
    if (s.type === "done") {
      ...
      // El paso anterior ya les hizo elegir canal. Aquí NO se repite el que descartaron:
      // solo queda el enlace de recuperación del que eligieron, por si no se abrió la
      // pestaña. (En la rama sin web el CTA ya llevaba al calendario.)
      ...
      stage.innerHTML = '<div class="g4uq-center"><span class="g4uq-eyebrow">¡Listo!</span>' +
        (noweb2
          ...
          : (doneReady
            ? '<h2>Tu Trust Score está listo.</h2><p class="g4uq-sub">' + (viaCal ? 'Te lo enseñamos en la llamada. ' : 'Te lo enviamos por WhatsApp ahora mismo. ') + recuperar + '</p>'
            : '<h2>' + CFG.done.title + '</h2><p class="g4uq-sub">' + (viaCal ? 'Te lo enseñamos en la llamada. ' : CFG.done.sub + ' ') + recuperar + '</p>')) +
        (viaCal
  ...
  function submit() {
    ...
    // Le mandamos también el `source`: el servidor empuja después a GHL, y si fijara uno
    // propio machacaría la atribución de la pieza (es lo que hacía el bridge viejo con
    // "trust-bridge"). Mandándoselo, puede reenviarlo en vez de inventárselo.
    if (C.TRUST && S.web) {
      ...
      try {
        fetch(C.TRUST + "/quiz-lead", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            session: S.session, email: S.email, nombre: S.nombre, apellido: "",
            empresa: S.empresa, phone: S.telefono, web: S.web, source: CFG.source
          })
        });
      } catch (e) {}
    }
