...
export default async (req: Request, _context: Context) => {
  ...
  if (!ANTHROPIC_KEY) {
    return new Response(JSON.stringify({ error: "ANTHROPIC_API_KEY not configured" }), { status: 500, headers: { ...CORS_HEADERS, "Content-Type": "application/json" } });
  }
  ...
  try {
    ...
    if (action === "scrape-news") {
      ...
      try {
        ...
        return new Response(JSON.stringify({ items }), { headers: { ...CORS_HEADERS, "Content-Type": "application/json" } });
      } catch {
        return new Response(JSON.stringify({ items: [], error: "Error fetching news" }), { headers: { ...CORS_HEADERS, "Content-Type": "application/json" } });
      }
    }
...
Genera el JSON completo de la newsletter.`;
    ...
    if (!res.ok) {
      ...
      return new Response(JSON.stringify({ error: `Anthropic error: ${err}` }), { status: 500, headers: { ...CORS_HEADERS, "Content-Type": "application/json" } });
    }
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: "Error parsing AI response", raw: text.slice(0, 500) }), { status: 500, headers: { ...CORS_HEADERS, "Content-Type": "application/json" } });
    }

    return new Response(JSON.stringify({ content: parsed }), { headers: { ...CORS_HEADERS, "Content-Type": "application/json" } });
  } catch (err: any) {
